...
void main() {
  for (Element element in querySelectorAll(".slideshow")) {
    if (!element.classes.contains("ignore")) slideshows
...
class Slideshow {
  ...
  Element _slideshowContainer,
      _currentSlide,
      _selectedThumbnail,
      _currentSlideText;
  num _pauseDuration;

  Slideshow(DivElement container, {num pauseDuration: 5000})
      : _pauseDuration = pauseDuration {
    if (container.children.every((Element element) =>
        element is ImageElement && element.classes.contains("slide"))) {
      ...
      container.children
        ..add(_slideshowContainer = new DivElement()
          ...
          ..children.add(
              _currentSlide = new ImageElement()..classes.add("currentSlide"))
      ...
      _slideshowContainer.onClick.listen((event) {
        if (_selectedThumbnail.attributes["href"] != null) window.location
            .assign(_selectedThumbnail.attributes["href"]);
      });
  ...
  void _nextImage() {
    ...
    if (index == _thumbnails.length - 1) _changeImage(_thumbnails.first);
    else _changeImage(_thumbnails[index + 1]);
    _initTimer();
  }

  void _previousImage() {
    num index = _thumbnails.indexOf(_selectedThumbnail);
    if (index == 0) _changeImage(_thumbnails.last);
    else _changeImage(_thumbnails[index - 1]);
    _initTimer();
  }

  void _changeImage(ImageElement thumbnail) {
    if (thumbnail == _selectedThumbnail) return;

    if (_currentSlide.attributes["src"] != "") {
      ShowHide
          .begin(ShowHideAction.HIDE, _currentSlide,
              duration: 500, effectTiming: EffectTiming.ease)
          .then((result) => _currentSlide.attributes["src"] = thumbnail.src);
    } else {
      _currentSlide.attributes["src"] = thumbnail.src;
    }

    ShowHide.show(_currentSlide, effect: new FadeEffect(), duration: 500);

    if (thumbnail.classes.contains("mouseOver")) {
      setTimer(false);
    }
    if (_selectedThumbnail != null) {
      _selectedThumbnail.classes.remove("selectedThumbnail");
    }

    _selectedThumbnail = thumbnail;
    _selectedThumbnail.classes.add("selectedThumbnail");

    if (_currentSlideText.text != "") {
      ShowHide
          .begin(ShowHideAction.HIDE, _currentSlideText,
              duration: 500, effectTiming: EffectTiming.ease)
          .then((result) {
    ...
    if (thumbnail.attributes["href"] != null) {
      _slideshowContainer.classes.add("link");
    } else {
      _slideshowContainer.classes.remove("link");
    }
  ...
  void setTimer(bool f) {
    if (_timer != null) if (f) _initTimer();
  ...
  void _initTimer() {
    ...
    _timer = new Timer(
        new Duration(milliseconds: _pauseDuration), () => _nextImage());
  }
}
